...
class ScheduleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Zona horaria de Madrid (Europa/Madrid)
    private val madridZoneId = ZoneId.of("Europe/Madrid")
    ...
    fun updateCurrentTime() {
        _currentTimeIndicator.value = getMadridTime()
    }

    // Obtener la hora actual en Madrid (GMT+2)
    private fun getMadridTime(): LocalTime {
        val now = ZonedDateTime.now(madridZoneId)
